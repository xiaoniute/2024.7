#ifndef  _TTALINK_CONVERSIONS_H_
#define  _TTALINK_CONVERSIONS_H_

#ifndef TTALINK_NO_CONVERSION_HELPERS
...
/**
 * @file ttalink_conversions.h
 *
 * These conversion functions follow the NASA rotation standards definition file
 * available online.
 *
 * Their intent is to lower the barrier for TTAlink adopters to use gimbal-lock free
 ...
 */
TTALINK_HELPER void ttalink_quaternion_to_dcm(const float quaternion[4], float dcm[3][3])
{
 ...
 */
TTALINK_HELPER void ttalink_dcm_to_euler(const float dcm[3][3], float* roll, float* pitch, float* yaw)
{
    float phi, theta, psi;
    theta = asin(-dcm[2][0]);
 ...
 * @param yaw the yaw angle in radians
 */
TTALINK_HELPER void ttalink_quaternion_to_euler(const float quaternion[4], float* roll, float* pitch, float* yaw)
{
    float dcm[3][3];
    ttalink_quaternion_to_dcm(quaternion, dcm);
    ttalink_dcm_to_euler((const float(*)[3])dcm, roll, pitch, yaw);
}
 ...
 * @param quaternion a [w, x, y, z] ordered quaternion (null-rotation being 1 0 0 0)
 */
TTALINK_HELPER void ttalink_euler_to_quaternion(float roll, float pitch, float yaw, float quaternion[4])
{
 ...
 * @param quaternion a [w, x, y, z] ordered quaternion (null-rotation being 1 0 0 0)
 */
TTALINK_HELPER void ttalink_dcm_to_quaternion(const float dcm[3][3], float quaternion[4])
{
 ...
 * @param dcm a 3x3 rotation matrix
 */
TTALINK_HELPER void ttalink_euler_to_dcm(float roll, float pitch, float yaw, float dcm[3][3])
{
...
#endif // TTALINK_NO_CONVERSION_HELPERS

#endif // _TTALINK_CONVERSIONS_H_
