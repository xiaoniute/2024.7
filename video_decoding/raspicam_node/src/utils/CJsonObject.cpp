...
std::string CJsonObject::ToFormattedString() const
{
    ...
    while (strJsonData.find('\t') != -1) //find返回-1表示字符串中已经不存在特定字符
        strJsonData.erase(strJsonData.find('\t'),1);//抹去查找到'\t'

    while (strJsonData.find('\n') != -1) //find返回-1表示字符串中已经不存在特定字符
        strJsonData.erase(strJsonData.find('\n'),1);//抹去查找到'\t'
...
bool CJsonObject::Add(const std::string& strKey, int64_t llValue)
{
    cJSON* pFocusData = NULL;
    if (m_pJsonData != NULL)
    {
...
bool CJsonObject::Add(const std::string& strKey, uint64_t ullValue)
{
    cJSON* pFocusData = NULL;
    if (m_pJsonData != NULL)
    {
...
bool CJsonObject::GetFatherItem(const std::string& strKey, std::string& strValue, CJsonObject& oJsonObject) const
{
    cJSON* pJsonStruct = NULL;
    if (m_pJsonData != NULL)
    {
